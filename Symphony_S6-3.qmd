...
CSTGates <- file.path(Computer, "Symphony S6-3.csv")
...
RepairAria <- Data %>% dplyr::filter(instrument %in% "Symphony S6-3")
...
TheColumnNames <- TheIntermediate[str_detect(TheIntermediate, "-A")]
    ...
    UVGains <- TheColumnNames[3:12]  
    VioletGains <- TheColumnNames[13:26]
    BlueGains <- TheColumnNames[27:35]
    YellowGreenGains <- TheColumnNames[36:44]
    RedGains <- TheColumnNames[45:50]
...
TheColumnNames <- TheColumnNames[str_detect(TheColumnNames, "-A")]
    ...
    UVGains <- TheColumnNames[2:11]  
    VioletGains <- TheColumnNames[12:25]
    BlueGains <- TheColumnNames[26:34]
    YellowGreenGains <- TheColumnNames[35:43]
    RedGains <- TheColumnNames[44:49]
...
Filename <- paste0("QCPlots_Symphony S6-3")

PDF <- Utility_Patchwork(x=PDFPlots, filename=Filename, returntype="pdf", outfolder=MainFolder, thecolumns=1)
```

## {.sidebar}
Dashboard data for the **BD Symphony S6-3** last updated on **`r TheDate`**
