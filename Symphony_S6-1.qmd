...
BDSummary <- function (x, Instrument, detectorType = "-A"){
  ...
  if (Instrument == "Fortessa"){
    ...
    UVNames <- TheColumnNames[25:32]  
    VioletNames <- TheColumnNames[16:24]
    BlueNames <- TheColumnNames[3:8]
    YellowGreenNames <- TheColumnNames[9:12]
    RedNames <- TheColumnNames[13:15]
  }
...
TheList <- "Symphony S6-1"
ArchiveCSV <- read.csv(list.files(file.path(MainFolder,TheList,"Archive"), pattern = "Holistic", full.names = T))
#MainFolder <- "/home/david/Documents/InstrumentQC/data"

CSTGates <- file.path(Computer, "Symphony S6-1.csv")
...
TheColumnNames <- TheIntermediate[str_detect(TheIntermediate, "-A")]
 ...
 UVGains <- TheColumnNames[26:32]  
    VioletGains <- TheColumnNames[17:25]
    BlueGains <- TheColumnNames[3:8]
    YellowGreenGains <- TheColumnNames[9:12]
    RedGains <- TheColumnNames[13:16]
...
TheColumnNames <- TheIntermediate[str_detect(TheIntermediate, "-A")]
 ...
 UVGains <- TheColumnNames[25:32]  
    VioletGains <- TheColumnNames[16:24]
    BlueGains <- TheColumnNames[3:8]
    YellowGreenGains <- TheColumnNames[9:12]
...
TheColumnNames <- TheColumnNames[str_detect(TheColumnNames, "-A")]
 ...
 UVGains <- TheColumnNames[24:31]  
    VioletGains <- TheColumnNames[15:23]
    BlueGains <- TheColumnNames[2:7]
    YellowGreenGains <- TheColumnNames[8:11]
    RedGains <- TheColumnNames[12:16]
