...
The5L <- list.files(Location, pattern="ArchivedDataAurora-1", recursive = TRUE, full.names = TRUE)
The5L <- read.csv(The5L[[1]], check.names=FALSE) %>% 
    select(DateTime, Temperature) %>% mutate(Instrument="Aurora-1")
The4L <- list.files(Location, pattern="ArchivedDataAurora-2", recursive = TRUE, full.names = TRUE)
The4L <- read.csv(The4L[[1]], check.names=FALSE) %>% 
    select(DateTime, Temperature) %>% mutate(Instrument="Aurora-2")

TheTimes <- rbind(The5L, The4L)
```
...
The5L <- list.files(Location, pattern="BeadDataAurora-1", recursive = TRUE, full.names = TRUE)
The5L <- read.csv(The5L[[1]], check.names=FALSE) %>% 
    select(DateTime)
The4L <- list.files(Location, pattern="BeadDataAurora-2", recursive = TRUE, full.names = TRUE)
The4L <- read.csv(The4L[[1]], check.names=FALSE) %>% 
    select(DateTime)
...
The4L <- list.files(MainFolder, pattern="ApplicationDataAurora-2", full.names=TRUE, recursive=TRUE)
The4L <- read.csv(The4L, check.names=FALSE) |> dplyr::mutate(Instrument = "Aurora-2")
Data <- rbind(The5L, The4L)
```
...
```{r}
#| echo: false
The4L <- Luciernaga:::UsagePlot(data=Data, TheInstrument="Aurora-2")
```
