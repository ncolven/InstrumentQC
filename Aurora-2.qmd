...
```{r}
MFI_5L <- Luciernaga:::CurrentData(x="Aurora-2", MainFolder=MainFolder, type = "MFI")
Gain_5L <- Luciernaga:::CurrentData(x="Aurora-2", MainFolder=MainFolder, type = "Gain")
...
Repair5L <- Data %>% filter(instrument %in% "Aurora-2")
```

```{r}
The5L <- Luciernaga:::VisualQCSummary(x=Gain_5L)
SmallTable <- Luciernaga:::SmallTable(The5L)
```
...
Filename <- paste0("QCPlots_Aurora-2")

PDF <- Utility_Patchwork(x=PDFPlots, filename=Filename, returntype="pdf", outfolder=MainFolder, thecolumns=1)
```

## {.sidebar}
Dashboard data  for the **Cytek Aurora-2** last updated on **`r TheDate`**

**Note:** Cytek QC Bead Lots were switched on 04-09-2026. MFI change on this date was expected.
